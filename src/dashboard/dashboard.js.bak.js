    ...
    for (let trackItem of tracks.items){
        ...
        if(!(id===nowPlaying.id)) {
         ...
         playTrackNoButton.onclick = function(event){
            ...
            console.log("playlist passed on onTrackPlayButton: ", playlist);
            onTrackPlayButtonClick(event,image,name, artist,duration_ms,previewURL,track, playlist);  
         }
...
const loadNowPlayingDefault = async()=>{
    ...
    for (let trackItem of playlists.items){
        if(index >1){break}
        var {id} = trackItem;
        nowPlayingPlaylist = trackItem;
        // console.log("nowPlayingPlaylist inside loadNowPlayingDefault : ", trackItem);
        index++;       
    }
    console.log("id: ", id);
